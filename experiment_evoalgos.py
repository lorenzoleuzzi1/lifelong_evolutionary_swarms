...
class LifelongEvoSwarmExperiment:
    ...
    def __init__(self,
        ...
        self.id_best = []
        self.generations_stats = []

        self._penalty_functional = {}
    ...
    def _run_episode(self, genome, config, env, regularization = None):
        ...
        for seed in environment_seeds:
            ...
            while True:
                ...
                if self.regularization_retention == "functional":
        ...
        if self.regularization_retention == "functional":
            self._penalty_functional[genome_id] = penalty_functional / len(environment_seeds) / env.duration
            print("penalty", genome_id, self._penalty_functional)
    ...
    def _evaluate_genomes_batch(self, genomes, config, env):
        ...
        for genome in genomes:
            ...
            results.append((genome[0], fitness))
        
        return results

    def _calculate_fitness_neat(self, genomes, config):
        
        population_stats = {} # key: id, value: species, fitness, adjusted fitness, retention fitness
        
        self.env.target_color = self.target_color

        if self.regularization_retention == "functional":
            self._penalty_functional = {genome_id: 0.0 for genome_id, _ in genomes}
        ...
        fitness_best = max([genome.fitness for _, genome in genomes])
        self.id_best.append([genome_id for genome_id, genome in genomes if genome.fitness == fitness_best][0])
        
        # ----- REGULARIZATION -----
        if self.regularization_retention is not None and self.best_individual is not None:
            # Save max fitness without penalty
            self.fitness_no_penalty.append(fitness_best)
            ...
            for genome_id, genome in genomes:
                ...
                # TODO: functional regularization (check)
                if self.regularization_retention == "functional":
                    print("fun", self._penalty_functional)
                    reg_penalty = self._penalty_functional[genome_id] * self.reg_lambdas.get('functional')
    ...
    def _save_results(self):
        ...
        if self.evolutionary_algorithm == "neat":
            ...
            with open(f"results/{self.experiment_name}/generation_statistics.json", "w") as f:
